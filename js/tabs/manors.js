...
const TabManors = {
  ...
  render() {
    ...
    const tabsHtml = keys.map(k => {
      ...
      return `<button class="manor-tab${this._current===k?' active':''}" onclick="TabManors.selectManor('${k}')">
        <span class="hh-dot" style="background:${col};"></span>${k}
      </button>`;
    }).join('');
  ...
  selectManor(key) {
    ...
    document.querySelector(`.manor-tab[onclick*="'${key}'"]`)?.classList.add('active');
    this._renderManor();
  },
